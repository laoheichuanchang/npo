...
function formatAction(value, row, index) {
	...
	var str = '';
	if (isEditOrgPermission) {
		str += '<a href="javascript:onEdit()">编辑</a>';
	}
	if (isDeleteOrgPermission) {
		if(str != ''){
			str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
		}
...
function onAdd(){
	var row = $('#treegrid').treegrid('getSelected');
 	if (row){
		...
		$('#form').form('clear');
		url = path+"/org/save?pid="+id;
		mesTitle = '新增组织成功';
 	}else{
 		$.messager.alert('提示', '请选择新增组织的父节点！', 'error');
 	}
}
...
function onEdit(){
 	...
 	if (row){
	 	...
	 	$('#form').form('load',row);//这句话有问题，第一次加载时正确的，第二次就出错了，还保持第一次的数据
	 	url = path+"/org/save?id="+id;
	 	mesTitle = '编辑组织成功';
 	}else{
